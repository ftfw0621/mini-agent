...
到这一步 `agent.ts` 的 `main()` 已经太挤了——它一个人扛着「读输入、跑循环、重试、退出善后」四件事。所以今天做这门课的**第一次拆分**:把**循环本身**从 `agent.ts` 搬进新文件 `src/loop.ts`,`agent.ts` 只留「读输入 → 起循环 → 按终态退出」的外壳。注意这不是又一个凭空冒出来的文件——它就是 Day1/Day2 那个 `for` 循环**搬了个家**,然后在新家里长成了状态机。以后你会反复看到这个动作:**一个文件长太大、混了两种职责,就按职责拆出去。**
...
指数退避(500ms → 1s → 2s → …)大家都懂,**抖动(±25% 随机)**才是容易漏的:服务器一抖,全世界的客户端同一毫秒失败;不加抖动,它们会在同一毫秒集体重试——这叫惊群(thundering herd),等于对一个已经踉跄的服务器发起 DDoS。本章开头那段重试日志里 588ms、1052ms 这两个不整的数字,就是抖动在工作。
